...
void ge_parallel(float *h_A, float *h_row_echelon_A, int N, int M, float *X) {
	...
	for (int i=0; i<N; i++) {
		...
		thrust::for_each(th_A_t+idx(i,i,N), th_A_t+idx(i+1,0,N), absolute<float>());
		thrust::device_vector<float>::iterator iter = 
			thrust::max_element(th_A_t+idx(i,i,N), th_A_t+idx(i+1,0,N));
...
int main() {
	...
	if (1) {
		ge_cpu((float *) A, (float *) row_echelon_A, N, M, X);
	}

	if (0) {
		ge_parallel((float *) A, (float *) row_echelon_A, N, M, NULL);
		cudaDeviceSynchronize();
	}
